...
## Model Hub
Cortex also offers a [Cortex Hub](https://huggingface.co/cortexhub) on Hugging Face, featuring a selection of models optimized for use with Cortex and versions for each engine provided by Cortex. These models include a `model.yaml` file containing presets and configurations tailored for optimal performance with the `onnx` engine.

Here's an example of the `model.yaml` file for `llama3 8B-onnx`:
```yaml
name: Llama 3
model: llama3:8B
version: 1
