...
Cortex is a C++ command-line interface (CLI) that serves as an alternative to Ollama, capable of running with three different engines: `llama.cpp`, `ONNX`, and `TensorRT-LLM`.
...
Cortex supports the following list of [Built-in Models](/models):
