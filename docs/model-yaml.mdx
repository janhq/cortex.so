...
Cortex automates model execution by utilizing the parameters specified in the `model.yaml` file. Therefore, Cortex requires a `model.yaml` file to run models. The `yaml` files are flat, portable, and contain necessary runtime parameters and model settings. `model.yaml` serves as a model configuration file and provides instructions on how Cortex should run the models.
