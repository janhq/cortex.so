...
Here is an example of `model.yaml` format that would run the `openhermes-2.5` model with `llamacpp` engine:
```yaml
# Cortex Meta
name: 
model: 
...
:::info
The `model.yaml` file format varies for each model type. To view the specific format for `model.yaml` for different model types, refer to:
...
:::info
Model presets override values in `model.yaml`. If presets are available, Cortex uses them. Otherwise, it defaults to `model.yaml` values.
:::
