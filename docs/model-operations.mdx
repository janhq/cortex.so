...
### Model Hub 
To enhance user experience, Cortex has curated a selection of models and their configurations, which can be found in the [Cortex Hub](https://huggingface.co/cortexhub) on Hugging Face.
:::info
...
Cortex looks for a `model.yaml` to run models. Otherwise, it is generated from model metadata. The `yaml` files are flat, portable, and contain all the parameters needed to run models.
